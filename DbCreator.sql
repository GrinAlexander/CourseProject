...
GO
CREATE VIEW ЗаказВкладкаView AS
	SELECT Деталь.название, Деталь.артикул, Деталь.категория, Заказ.количество, Заказ.сумма, Авто.модель FROM Деталь
	JOIN Заказ ON Заказ.id_детали = Деталь.id
	JOIN Авто ON Авто.id = Деталь.id_авто

GO
CREATE VIEW ВручкаView AS
	SELECT YEAR(Продажа.дата) AS [Год], SUM(Заявка.сумма) AS [Выручка] FROM Заявка
	JOIN Продажа ON Продажа.id_заявки = Заявка.id
	GROUP BY YEAR(Продажа.дата)

GO
CREATE VIEW ПродажиView AS
	SELECT Деталь.название, Деталь.артикул, Продажа.дата, Заявка.сумма FROM Деталь
	JOIN Заявка ON Деталь.id = Заявка.id_детали
	JOIN Продажа ON Продажа.id_заявки = Заявка.id